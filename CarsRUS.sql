...
CREATE TABLE `VehicleAttributes` (
  ...
  `value` varchar(20) NOT NULL COMMENT 'For packages; use \'package\' ',
  `parent_package` varchar(20) NOT NULL,
...
ALTER TABLE `VehicleAttributes` ADD FOREIGN KEY (`parent_package`) REFERENCES `VehicleAttributes` (`attribute`) ON DELETE CASCADE ON UPDATE CASCADE;
